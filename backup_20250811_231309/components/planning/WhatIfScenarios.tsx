...
export const WhatIfScenarios = ({
  ...
  const currentPortfolioValue = useCurrentPortfolioValue();
  const monthlyContribution = useMonthlyContribution();
  
  // Component state
  const [currentScenario, setCurrentScenario] = useState<ScenarioInputs>(() => ({
    ...defaultScenario,
    location: user?.location?.state || 'California',
