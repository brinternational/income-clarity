...
const MOCK_DIVIDEND_STOCKS: DividendStock[] = [
  {
    symbol: 'JEPI',
    ...
    payoutRatio: 95.2,
    growthRate5Y: 8.3,
    lastExDate: '2024-07-30',
    nextExDate: '2024-08-30',
  ...
  },
  {
    ...
    payoutRatio: 98.7,
    growthRate5Y: -2.1,
    lastExDate: '2024-07-29',
    nextExDate: '2024-08-29',
    paymentFrequency: 'Monthly',
    sector: 'Technology',
    qualityScore: 78,
  ...
  },
  {
    ...
    payoutRatio: 56.8,
    growthRate5Y: 12.1,
    lastExDate: '2024-08-15',
    nextExDate: '2024-11-15',
  ...
  },
  {
    ...
    payoutRatio: 65.2,
    growthRate5Y: 9.7,
    lastExDate: '2024-08-20',
    nextExDate: '2024-11-20',
    paymentFrequency: 'Quarterly',
    sector: 'Diversified',
    qualityScore: 89,
    sustainabilityRating: 'High',
...
const INTELLIGENCE_INSIGHTS: IntelligenceInsight[] = [
  {
    ...
    priority: 'High',
    title: 'High-Quality Dividend Growth Opportunity',
    description: 'SCHD shows strong fundamentals with 96% quality score and 12.1% 5-year growth rate',
    action: 'Consider increasing allocation to SCHD for sustainable dividend growth',
    potentialImpact: 850,
    confidence: 92,
  ...
  },
  {
    ...
    category: 'Risk',
    priority: 'Medium',
    title: 'High Payout Ratio Alert',
    description: 'QYLD and RYLD have payout ratios >94%, indicating limited sustainability buffer',
    action: 'Monitor closely and consider position sizing adjustments',
    potentialImpact: -320,
    confidence: 88,
  ...
  },
  {
    ...
    priority: 'High',
    title: 'Tax Efficiency Enhancement',
    description: 'Portfolio weighted toward qualified dividends can benefit from Puerto Rico tax advantage',
    action: 'Optimize holdings for 0% qualified dividend tax rate',
    potentialImpact: 1250,
    confidence: 96,
    timeframe: 'Immediate'
  },
  {
    ...
    description: 'Technology and financial services represent 68% of dividend income',
    action: 'Consider diversification into utilities, REITs, or consumer staples',
    potentialImpact: 180,
    confidence: 75,
...
export const DividendIntelligenceEngine: React.FC<DividendIntelligenceEngineProps> = ({
  ...
  const portfolioMetrics = useMemo(() => {
    const totalValue = MOCK_DIVIDEND_STOCKS.reduce((sum, stock) => sum + (stock.marketCap / 1000000), 0);
    const weightedYield = MOCK_DIVIDEND_STOCKS.reduce((sum, stock) => 
      sum + (stock.currentYield * (stock.marketCap / 1000000)), 0
    ) / totalValue;
    const avgQualityScore = MOCK_DIVIDEND_STOCKS.reduce((sum, stock) => sum + stock.qualityScore, 0) / MOCK_DIVIDEND_STOCKS.length;
    const avgGrowthRate = MOCK_DIVIDEND_STOCKS.reduce((sum, stock) => sum + stock.growthRate5Y, 0) / MOCK_DIVIDEND_STOCKS.length;
    ...
    return {
      ...
      avgQualityScore,
      avgGrowthRate,
      totalHoldings: MOCK_DIVIDEND_STOCKS.length
    };
  }, []);
  ...
  const filteredStocks = useMemo(() => {
    let filtered = MOCK_DIVIDEND_STOCKS;
          ...
          <div className="space-y-6">
            ...
            <AnimatePresence>
              {selectedStock && (
                ...
                >
                  {(() => {
                    const stock = MOCK_DIVIDEND_STOCKS.find(s => s.symbol === selectedStock);
