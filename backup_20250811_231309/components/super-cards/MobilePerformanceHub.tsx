  ...
  const topPerformersContent = performanceInsights?.topPerformers ? (
    <div className="space-y-3">
      {performanceInsights.topPerformers.slice(0, 3).map((holding, index) => (
        <div key={holding.id} className="flex items-center justify-between p-4 bg-prosperity-50 rounded-lg border border-prosperity-200">
  ...
  return (
    <MobileCardLayout
      ...
      onSectionToggle={(sectionId, isCollapsed) => {
        // console.log(`Section ${sectionId} ${isCollapsed ? 'collapsed' : 'expanded'}`);
      // }}
