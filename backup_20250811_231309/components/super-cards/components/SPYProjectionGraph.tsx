...
import { SPYProjectionData, ProjectionPeriod } from '@/hooks/useSPYProjections';

interface SPYProjectionGraphProps {
  projections: SPYProjectionData[];
  selectedPeriod: ProjectionPeriod;
  onPeriodSelect: (period: ProjectionPeriod) => void;
  isLoading?: boolean;
  getScenarioColor: (scenario: 'Conservative' | 'Expected' | 'Optimistic') => string;
}
...
const SPYProjectionGraphComponent = ({
  ...
  const selectedProjection = useMemo(() => {
    return projections.find(p => p.period === selectedPeriod);
  ...
  const chartData = useMemo(() => {
    ...
    const maxValue = Math.max(...scenarios.map(s => Math.max(s.portfolioReturn, s.spyReturn))) * 100;
    const minValue = Math.min(...scenarios.map(s => Math.min(s.portfolioReturn, s.spyReturn))) * 100;
    const range = Math.max(Math.abs(maxValue), Math.abs(minValue));
    
    return scenarios.map(scenario => ({
      ...scenario,
      portfolioHeight: Math.max(10, (Math.abs(scenario.portfolioReturn * 100) / range) * 100),
      spyHeight: Math.max(10, (Math.abs(scenario.spyReturn * 100) / range) * 100),
      outperformanceHeight: Math.max(5, (Math.abs(scenario.outperformance * 100) / (range * 0.5)) * 50)
    }));
  }, [selectedProjection]);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            <h5 className="font-semibold text-slate-800 flex items-center">
              ...
              {periodLabels[selectedProjection.period]} Outlook
            </h5>
          ...
          <div className="text-right">
            <div className="text-lg font-bold text-primary-600">
              {selectedProjection.probabilityOfOutperformance >= 0.5 ? (
                <div className="flex items-center space-x-1">
                  <TrendingUp className="w-4 h-4" />
                  <span>{(selectedProjection.probabilityOfOutperformance * 100).toFixed(0)}%</span>
                </div>
              ) : (
                <div className="flex items-center space-x-1">
                  <TrendingDown className="w-4 h-4" />
                  <span>{(100 - selectedProjection.probabilityOfOutperformance * 100).toFixed(0)}%</span>
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="text-center p-3 bg-white/50 rounded-lg border border-white/60">
            <div className="text-xs text-slate-600 mb-1">Expected Alpha</div>
            <div className={`text-lg font-bold ${
              selectedProjection.expectedOutperformance >= 0 ? 'text-prosperity-600' : 'text-alert-600'
            }`}>
              {selectedProjection.expectedOutperformance >= 0 ? '+' : ''}{(selectedProjection.expectedOutperformance * 100).toFixed(1)}%
            </div>
          </div>
          
          <div className="text-center p-3 bg-white/50 rounded-lg border border-white/60">
            <div className="text-xs text-slate-600 mb-1">Target Portfolio</div>
            <div className="text-lg font-bold text-primary-600">
              ${selectedProjection.targetPrice.portfolio.toFixed(0)}
            </div>
          </div>
          
          <div className="text-center p-3 bg-white/50 rounded-lg border border-white/60">
            <div className="text-xs text-slate-600 mb-1">Target SPY</div>
            <div className="text-lg font-bold text-wealth-600">
              ${selectedProjection.targetPrice.spy.toFixed(0)}
            </div>
          </div>
          ...
          <div className="text-center p-3 bg-white/50 rounded-lg border border-white/60">
            ...
            <div className="text-lg font-bold text-secondary-600">
              {selectedProjection.milestones.length}
            </div>
          </div>
        </div>
      </motion.div>
                  ...
                  <div className="flex space-x-3 items-end">
                    ...
                    <div className="flex flex-col items-center space-y-1">
                      ...
                      <div className="text-xs font-bold text-primary-600">
                        {scenario.portfolioReturn >= 0 ? '+' : ''}{(scenario.portfolioReturn * 100).toFixed(0)}%
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center space-y-1">
                      ...
                      <div className="text-xs font-bold text-wealth-600">
                        {scenario.spyReturn >= 0 ? '+' : ''}{(scenario.spyReturn * 100).toFixed(0)}%
                      </div>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {selectedProjection.scenarios.map((scenario, index) => (
              <motion.div
                ...
                className={`p-4 rounded-lg border-2 ${getScenarioColor(scenario.name)}`}
              >
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <div className="p-6">
          <div className="space-y-4">
            {selectedProjection.milestones.map((milestone, index) => (
              <motion.div
                key={milestone.date}
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-xs text-slate-600">
                        Confidence: {(milestone.probability * 100).toFixed(0)}%
                      </div>
                    </div>
                    
                    <div className="text-right">
                      <div className="text-sm font-bold text-primary-600">
                        ${milestone.portfolioValue.toFixed(0)}
                      </div>
                      <div className="text-xs text-wealth-600">
                        SPY: ${milestone.spyValue.toFixed(0)}
                      </div>
                    </div>
                  </div>
                </div>
