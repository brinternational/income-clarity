...
import { InlineEditHolding } from './InlineEditHolding';
...
interface EnhancedHoldingsListProps {
  ...
  onInlineUpdate?: (holdingId: string, field: string, value: number) => Promise<void>;
}

export function EnhancedHoldingsList({ 
  holdings, 
  onEdit, 
  onDelete, 
  onQuickPurchase, 
  ...
  onRefreshPrices,
  onInlineUpdate 
}: EnhancedHoldingsListProps) {
  const [sortField, setSortField] = useState<keyof Holding>('ticker');
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>('asc');
  const [refreshing, setRefreshing] = useState(false);
  const [editingCell, setEditingCell] = useState<{holdingId: string, field: string} | null>(null);
  ...
  const handleInlineEditStart = (holdingId: string, field: string) => {
    setEditingCell({ holdingId, field });
  };

  const handleInlineEditSave = async (holdingId: string, field: string, value: number) => {
    if (onInlineUpdate) {
      await onInlineUpdate(holdingId, field, value);
    }
    setEditingCell(null);
  };
        ...
        <div className="px-4 py-3 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          ...
          {onRefreshPrices && (
            <motion.button
              ...
              disabled={refreshing}
              className="px-3 py-1.5 text-sm bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white rounded-lg transition-colors flex items-center space-x-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                ...
                <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">
                  {onInlineUpdate ? (
                    <InlineEditHolding
                      holding={holding}
                      field="shares"
                      isEditing={editingCell?.holdingId === holding.id && editingCell?.field === 'shares'}
                      onEditStart={() => handleInlineEditStart(holding.id, 'shares')}
                ...
                <td className="px-4 py-3 text-sm text-gray-900 dark:text-white">
                  {onInlineUpdate ? (
                    <InlineEditHolding
                      holding={holding}
                      field="costBasis"
                      isEditing={editingCell?.holdingId === holding.id && editingCell?.field === 'costBasis'}
                      onEditStart={() => handleInlineEditStart(holding.id, 'costBasis')}
                      onSave={handleInlineEditSave}
                      onCancel={handleInlineEditCancel}
                    />
                  ) : (
                  ...
                  <div className="flex items-center justify-end space-x-1">
                    {onQuickPurchase && (
                      <motion.button
                        ...
                        className="p-1.5 text-gray-400 hover:text-green-600 dark:hover:text-green-400 transition-colors rounded focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-1"
                        title="Quick Add Purchase"
                        aria-label={`Quick add purchase for ${holding.ticker}`}
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {onRecordDividend && (
                      <motion.button
                        ...
                        className="p-1.5 text-gray-400 hover:text-purple-600 dark:hover:text-purple-400 transition-colors rounded focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-1"
                        title="Record Dividend Payment"
                        aria-label={`Record dividend for ${holding.ticker}`}
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <motion.button
                      ...
                      className="p-1.5 text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"
                      title="Edit Holding"
                    ...
                    <motion.button
                      ...
                      className="p-1.5 text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors rounded focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-1"
